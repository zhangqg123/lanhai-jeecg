...
function saveOption(options,i){
 ...
 function getDsLi(){
	...
	function mySearch() {
		...
		frameElement.api.opener.document.getElementById("_complexSqlbuilder").value = jsonData.toString();
		frameElement.api.opener['${tableName}search'].call();
	}
	...
	function searchReset() {
		...
		spanVal.html("<input name='cons[0].val' type='text' class='text conditionValue' style='width:165px;'>");
		$("#field").empty();
		//$("#_complexSqlbuilder").val(null);
		frameElement.api.opener.document.getElementById("_complexSqlbuilder").value = "";
		frameElement.api.opener.${tableName}search();
		resetTrNum();
	}
	//判断输入的是否为日期格式
	...
	function showMenu() {
		...
		//计算文本框相对DIV的位置
		var top = cityOffset.top+28;
		var left = cityOffset.left;
		$("#menuContent").css({top:top +"px",left:left+"px"}).slideDown("fast");
		...
		jQuery.ajax({  
			...
			success:function(data){
				if("input" == data.obj[0].stype) {
					...
					var selectContent = "<select name=\"cons["+i+"].val\" class=\"compareType conditionValue\" style=\"width: 171px\">";
					jQuery.ajax({
						type: 'POST',
						dataType : "json",
						data:{
		...
		jQuery.ajax({  
			...
			success:function(data){
				if("input" == data.obj[0].stype) {
					...
					var selectContent = "<select name=\"cons["+index+"].val\" class=\"compareType conditionValue\" style=\"width: 171px\">";
					jQuery.ajax({
						type: 'POST',
						dataType : "json",
						data:{
