...
<html>
 ...
 <body>
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="qywxLinksucaiController.do?doUpdate" tiptype="1">
						...
						<td class="value">
				          <select name="agentId" id="agentId">
				          		<c:forEach items="${agentList}" var="item">
				          			<option value="${item.id}" name="agentId" <c:if test="${weixinLinksucaiPage.agentId==item.id}">selected="selected"</c:if>>${item.name}</option>
<%-- 				          			<option value="${item.id}" name="agentId" >${item.name}</option> --%>
				          		</c:forEach>
				          </select>
							<span class="Validform_checktip"></span>
							<label class="Validform_label" style="display: none;">企业应用</label>
						</td>
					</tr>
					<%--
					<tr>
