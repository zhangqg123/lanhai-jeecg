...
<html>
 ...
 <body>
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="qywxLinksucaiController.do?doAdd" tiptype="1">
...
<%-- 				          			<option value="${item.get('id')}" name="agentId" <c:if test="${agentId==item.get('id')}">selected="selected"</c:if>>${item.get('name')}</option> --%>
				          			<option value="${item.id}" name="agentId" >${item.name}</option>
				          		</c:forEach>
				          </select>
							<span class="Validform_checktip"></span>
							<label class="Validform_label" style="display: none;">企业号应用</label>
					</td>
				</tr>
