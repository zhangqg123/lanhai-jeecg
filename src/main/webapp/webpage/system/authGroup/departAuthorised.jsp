...
<script type="text/javascript">
	var setting = {
		...
		async: {
			enable: true,
			url:"departAuthGroupController.do?setAuthority&gid=${gid}&pid=${pid}",
			dataFilter: filter				
		},
		callback: {
			...
			onClick: function (event, treeId, treeNode){
				if (null == treeNode.children) {
					...
					$('#operationListpanel').panel(
							"refresh",
							"departAuthGroupController.do?operationListForFunction&functionId="+ treeNode.id + "&groupId=" + groupId);
					$('#dataRuleListpanel').panel(
							"refresh",
							"departAuthGroupController.do?dataRuleListForFunction&functionId="+ treeNode.id + "&groupId=" + groupId);
				} else {
				}
			}
		}
	...
	function mysubmit() {
		var groupId = $("#groupId").val();
		var s = GetNode();
		doSubmit("departAuthGroupController.do?updateDepartAuthority&functions=" + s + "&groupId=" + groupId);
	}
	//获取选中节点
	function GetNode() {
