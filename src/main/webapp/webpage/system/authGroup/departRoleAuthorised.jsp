...
<script type="text/javascript">
	var setting = {
		...
		async: {
			enable: true,
			url:"departAuthGroupController.do?setRoleAuthority&gid=${gid}&pid=${departAgId}",
			dataFilter: filter				
		},
		callback: {
			...
			onClick: function (event, treeId, treeNode){
				if (null == treeNode.children) {
					...
					$('#operationListpanel').panel(
							"refresh",
							"departAuthGroupController.do?roleOperationListForFunction&functionId="+ treeNode.id + "&groupId=" + groupId);
					$('#dataRuleListpanel').panel(
							"refresh",
							"departAuthGroupController.do?roleDataRuleListForFunction&functionId="+ treeNode.id + "&groupId=" + groupId);
				} else {
				}
			}
		}
	...
	function mysubmit() {
		var groupId = $("#groupId").val();
		var s = GetNode();
		doSubmit("departAuthGroupController.do?updateDepartRoleAuthority&functions=" + s + "&groupId=" + groupId);
	}
	//获取选中节点
	function GetNode() {
