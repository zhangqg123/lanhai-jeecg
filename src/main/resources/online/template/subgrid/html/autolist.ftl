${config_iframe}
<script type="text/javascript" src="plug-in/easyui/extends/datagrid-detailview.js"></script>
...
function  detailFormatterFun(){
			return '<div class="orderInfoHidden" style="padding:2px;">'+
							...
							<#list subtablelist as sub >
							    <#if field['${sub}']?exists >
							    	'<div title="${field['${sub}'].head.content?if_exists?html}" style="padding:2px;">'+
			                            '<table class="${field['${sub}'].head.tableName?if_exists?html}tablelines" ></table>'+
			                            '</div>'+
	...
	<#list subtablelist as sub >
		<#if field['${sub}']?exists >
		    ...
		    <#list field['${sub}'].fieldList as subTableField >
		    	<#if subTableField.main_field?if_exists?html != ''><#assign mainids="${subTableField.field_name?if_exists?html}"></#if>
		    	<#assign subfiedsss=subfiedsss+"${subTableField.field_name?if_exists?html},">
			</#list>
			<#list field['${sub}'].hiddenFieldList as subTableField >
				 <#if subTableField.main_field?if_exists?html != ''><#assign mainids="${subTableField.field_name?if_exists?html}"></#if>
			      <#assign subfiedsss=subfiedsss+"${subTableField.field_name?if_exists?html},">
			</#list>
		    var ${field['${sub}'].head.tableName?if_exists?html}durl = 'cgAutoListController.do?datagrid&configId=${field['${sub}'].head.tableName?if_exists?html}&${mainids}='+row.id+'&field=${subfiedsss}&page=1&rows=100';
			var ${field['${sub}'].head.tableName?if_exists?html}tablelines = $(this).datagrid('getRowDetail',index).find('table.${field['${sub}'].head.tableName?if_exists?html}tablelines');
			${field['${sub}'].head.tableName?if_exists?html}tablelines.datagrid({
					singleSelect:true,
				    loadMsg:'正在加载',
				    fitColumns:true,
				    height:'180',
					...
					border:false,
					loadMsg:"",
					url: ${field['${sub}'].head.tableName?if_exists?html}durl,
					idField : 'id',  
					rownumbers : true,
					pagination : false, 
