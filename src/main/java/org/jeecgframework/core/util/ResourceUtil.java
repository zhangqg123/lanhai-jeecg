...
//		String requestPath = request.getRequestURI() + "?" + request.getQueryString();
	...
	public static final String getQyUserId() {
		HttpSession session = ContextHolderUtils.getSession();
		Object qyUserId = session.getAttribute(WeiXinConstants.QY_USERID);
		if(qyUserId!=null){
			return qyUserId.toString();
		}else{
			return null;
		}
	} 

	public static final String getQyUserid(HttpServletRequest request) {
		String qyuserid = request.getParameter("qyuserid");
		if(qyuserid!=null){
			return qyuserid;
		}else{
			return null;
